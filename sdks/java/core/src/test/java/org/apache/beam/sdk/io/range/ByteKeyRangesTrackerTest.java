...
public class ByteKeyRangesTrackerTest {
  ...
  private static final ByteKey FIRST_BEFORE_START_KEY = ByteKey.of(0x2);
  ...
  private static final ByteKey SECOND_BEFORE_END_KEY = ByteKey.of(0x33);
  ...
  public void testGetFractionConsumed() {
    ...
    tracker.tryReturnRecordAt(true, SECOND_BEFORE_END_KEY);
  ...
  public void testTryReturnRecordAt() {
    ...
    assertTrue(tracker.tryReturnRecordAt(true, SECOND_BEFORE_END_KEY));

    assertFalse(tracker.tryReturnRecordAt(true, SECOND_END_KEY)); // after end

    assertFalse(tracker.tryReturnRecordAt(true, SECOND_BEFORE_END_KEY)); // false because done
  }
  ...
  public void testTryReturnBeforeStartKey() {
    ByteKeyRangesTracker tracker = ByteKeyRangesTracker.of
        (ImmutableList.of(SECOND_RANGE, FIRST_RANGE));
    expected.expect(IllegalStateException.class);
    tracker.tryReturnRecordAt(true, FIRST_BEFORE_START_KEY);
  }
  ...
  public void testSplitAtPosition() {
    ...
    assertTrue(tracker.tryReturnRecordAt(true, FIRST_START_KEY));
    assertTrue(tracker.trySplitAtPosition(SECOND_BEFORE_END_KEY));
    assertEquals(SECOND_BEFORE_END_KEY, tracker.getStopPosition());
